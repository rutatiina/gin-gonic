...
func OrganizationOthers(c *gin.Context) {
...
// POST /books
// Create new book
func OrganizationStore(c *gin.Context) {
	/*
		// Validate input
		var input UserNew
	...
	*/
	...
	// Validate input
	var input models.OrganizationNew
	if err := c.ShouldBindBodyWith(&input, binding.JSON); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	var webUser models.Organization
...
// DELETE /books/:id
// Delete a book
func OrganizationDestroy(c *gin.Context) {
	// Get model if exist
	var model models.Organization
	if err := services.DB.Where("id = ?", c.Param("id")).First(&model).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Record not found!"})
...
func OrganizationVoidDestroyed(c *gin.Context) {
	// Get model if exist
	var model models.Organization
